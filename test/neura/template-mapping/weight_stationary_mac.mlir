...
module {
  func.func @ws_mac(%A: memref<2x2xi32>, %B: memref<2x1xi32>, %C: memref<2x1xi32>) {
    ...
    ^bb0(%a: memref<2x2xi32>, %b: memref<2x1xi32>, %c: memref<2x1xi32>):
      %a0 = "neura.load"() {memory_access = {kernel_input = 0 : i32, offsets = array<i64: 0, 2>}, placement = {x = 0 : i32, y = 2 : i32}} : () -> i32
      %m_1_2:2 = "neura.mac"(%a0) {placement = {x = 1 : i32, y = 2 : i32}} : (i32) -> (i32, i32)

      %a1 = "neura.load"() {memory_access = {kernel_input = 0 : i32, offsets = array<i64: 1, 3>}, placement = {x = 0 : i32, y = 1 : i32}} : () -> i32
      %m_1_1:2 = "neura.mac"(%a1, %m_1_2#0) {placement = {x = 1 : i32, y = 1 : i32}} : (i32, i32) -> (i32, i32)

      "neura.store"(%m_1_1#0) {memory_access = {kernel_input = 2 : i32, offsets = array<i64: 0, 1>}, placement = {x = 1 : i32, y = 0 : i32}} : (i32) -> ()
      neura.yield
    }
    return
  }
...
// MAPPING-NEXT:       %0 = "neura.load"() {dfg_id = 0 : i32, mapping_locs = [{id = 8 : i32, index_per_ii = 0 : i32, invalid_iterations = 0 : i32, resource = "tile", time_step = 0 : i32, x = 0 : i32, y = 2 : i32}], memory_access = {kernel_input = 0 : i32, offsets = array<i64: 0, 2>}} : () -> !neura.data<i32, i1>
...
// MAPPING-NEXT:       %2 = "neura.load"() {dfg_id = 1 : i32, mapping_locs = [{id = 4 : i32, index_per_ii = 0 : i32, invalid_iterations = 1 : i32, resource = "tile", time_step = 1 : i32, x = 0 : i32, y = 1 : i32}], memory_access = {kernel_input = 0 : i32, offsets = array<i64: 1, 3>}} : () -> !neura.data<i32, i1>
...
// MAPPING-NEXT:       "neura.store"(%5) {dfg_id = 9 : i32, mapping_locs = [{id = 1 : i32, index_per_ii = 0 : i32, invalid_iterations = 3 : i32, resource = "tile", time_step = 3 : i32, x = 1 : i32, y = 0 : i32}], memory_access = {kernel_input = 2 : i32, offsets = array<i64: 0, 1>}} : (!neura.data<i32, i1>) -> ()
...
// ASM-NEXT: PE(1,0):
// ASM-NEXT: {
// ASM-NEXT:   STORE, [NORTH, RED], [address(arg2, 0, 1)] (t=0, inv_iters=0)
// ASM-NEXT: } (idx_per_ii=0)
// ASM-EMPTY:
// ASM-NEXT: PE(0,1):
// ASM-NEXT: {
// ASM-NEXT:   LOAD, [address(arg0, 1, 3)] -> [EAST, RED] (t=0, inv_iters=0)
// ASM-NEXT: } (idx_per_ii=0)
// ASM-EMPTY:
// ASM-NEXT: PE(1,1):
// ASM-NEXT: {
// ASM-NEXT:   MUL_ADD, [WEST, RED], [value(arg1, 1)], [NORTH, RED] -> [SOUTH, RED] (t=0, inv_iters=0)
// ASM-NEXT: } (idx_per_ii=0)
// ASM-EMPTY:
// ASM-NEXT: PE(0,2):
// ASM-NEXT: {
