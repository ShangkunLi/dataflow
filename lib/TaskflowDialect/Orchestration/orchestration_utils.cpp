  ...
  for (int iter = 0; iter < kMaxIterations; ++iter) {
    ...
    for (TaskNode *task_node : sorted_tasks) {
      ...
      if (auto attr =
        ...
        active_replicas = std::max(1, static_cast<int>(attr.getInt()));
      }

      for (int replica = 0; replica < active_replicas; ++replica) {
...
void TaskScheduler::recordScheduleResult(const TaskMemoryGraph &graph) {
  ...
  for (const auto &task_node : graph.task_nodes) {
    ...
    task_result.duration = task_node->getDuration();
    task_result.end_time =
        task_result.start_time + task_node->placement.front().duration;
...
void TaskScheduler::updateScheduleTimeScale(const TaskMemoryGraph &graph) {
  ...
  // duration; TaskPipelineIntervalAnalyzer still uses the original
  // profile_info.duration values for throughput analysis.
